...
// Les value notifier permettent le lien entre les données courantes pour l'appli collectéees depuis le serveur (et donc font le lien avec les widget affichés)
...
void newObject(lastMesure) async {
  ...
  try {
    ...
    if(objectNameInput.isNotEmpty) { // Si l'utilisateur souhaite personaliser le nom de l'objet / a répondu
      // Get list of the user's scanned items
      final resultList = await pb.collection('objet').getFullList(
        filter: 'user = "$userId"', // Guillemets nécessaires autour des valeurs String
      );
      ...
      for (dynamic obj in objectsList) {
        if (objectNameInput == obj['nom_objet']) { // On suppose que les id ou nom de materiaux sont uniques pour chaque materiau et sont non vides
          objetId = obj['id'];
          contained = true;
        }
      }

      if (!contained) { // Si l'objet entré n'existe pas dans la bdd (not contained), l'ajouter
        Map<String, dynamic> objetInfo = {
          'user' : userId,
          'nom_objet' : objectNameInput,
        };
        final createdRecord = await pb.collection("objet").create(
          body: objetInfo
        );
        print(objetInfo);
      ...
      Map<String,dynamic> lastMesureRecord = lastMesure.toJson(); // On récupère l'élement sous forme de dico
...
Future<String> askUserObjectName() async {
  if (_currentInputCompleter != null && !_currentInputCompleter!.isCompleted) { // On a déjà eu un input et il n'est pas encore compélté
  ...
  late VoidCallback listener; // Déclaration d'une fonction listener initialisée ci-dessous
  listener = () {
    if (!displayInputObjectNotifier.value) { // Si jamais l'affichage du widget passe à false, donc que l'utilisateur a interargit avec le dialogue
      if (!completer.isCompleted) { // Si le nouveau completer a déjà été bien initialisé avant fermeture de l'autre ou qu'on est dans le cas où une valeur n'a pas déjà été entrée juste avant
        completer.complete(inputObjectNotifier.value); // Attribue la complétion du future à la valeur rentrée par l'user
      }
      displayInputObjectNotifier.removeListener(listener); // Fin de l'écoute de modification
      _currentInputCompleter = null; // Plus de compléteur en cours
    }
  };

  displayInputObjectNotifier.addListener(listener); // Début de l'écoute de l'action de l'user
